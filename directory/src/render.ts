...
/**
 * Cloudflare Web Analytics beacon token, set per request from the worker env.
 *
 * Cloudflare's automatic injection only rewrites proxied origin responses.
 * These pages are generated by a Worker, so there is no origin HTML to rewrite
 * and the tag has to come from us. The token is not a secret; it ships in the
 * page to every visitor.
 */
let beaconToken = "";

export function setBeaconToken(token: string): void {
  beaconToken = token || "";
}
